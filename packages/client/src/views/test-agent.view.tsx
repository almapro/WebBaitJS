...
import { connectClientWebsocket, clientWebsocketService } from "../services";
...
export const RowOptionsCell: FC<{ params: GridRenderCellParams<boolean>, clickedOnViewSubject: Subject<boolean>, deleteCmd: (cmdId: string) => void }> = ({ params, clickedOnViewSubject, deleteCmd }) => {
    ...
    <>
      ...
      <Menu
        ...
        {options.map((value, idx) => (
          <MenuItem key={idx} onClick={() => { setOpenMenu(false); value.action(); }}>{value.label}</MenuItem>
        ))}
      </Menu>
...
export const TestAgentView = () => {
  ...
  const handleConnectDisconnect = async () => {
    if (clientWebsocketService && connected) {
      ...
      if (clientWebsocketService) {
        ...
        clientWebsocketService.on("cmd", (cmd: { cmd: string, cmdId: string }) => {
  ...
  const columns: GridColDef[] = [
    ...
    { field: "options", headerName: "", width: 65, hideSortIcons: true, disableColumnMenu: true, renderCell: (params) => (<RowOptionsCell deleteCmd={cmdId => setCmds(cmds => cmds.filter(cmd => cmd.cmdId !== cmdId))} params={params} clickedOnViewSubject={clickedOnViewSubject} />) },
  ];
